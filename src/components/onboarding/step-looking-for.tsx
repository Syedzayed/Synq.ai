...
const LOOKING_FOR_PRESETS = [
  "Developers", "Designers", "Researchers", "Founders",
  "Marketers", "Data Scientists", "Product Managers",
  "Investors", "Mentors", "Students", "Writers",
  "DevOps Engineers", "ML Engineers", "Business Leads",
];

interface StepLookingForProps {
  data: Pick<OnboardingData, "lookingFor">;
  onChange: (patch: Partial<OnboardingData>) => void;
  onNext: () => void;
  onBack: () => void;
}

export function StepLookingFor({ data, onChange, onNext, onBack }: StepLookingForProps) {
  const canProceed = data.lookingFor.length > 0;
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-2 text-[14px]" style={{ color: "#9e9890" }}>
          Synq will surface these people first.{data.lookingFor.length > 0 && <span className="ml-1 font-semibold" style={{ color: "#e07a5f" }}>{data.lookingFor.length} selected</span>}
        </p>
      </div>
      <TagSelector presets={LOOKING_FOR_PRESETS} selected={data.lookingFor} onChange={(lookingFor) => onChange({ lookingFor })} placeholder="Add a type..." />
